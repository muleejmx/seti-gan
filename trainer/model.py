...
### normalized data
with open('normalized_diag.pkl', 'rb') as f:
    data = pickle.load(f)
...
plt.plot(history.history['player_0_loss'], color='r') #Label with colors
plt.plot(history.history['player_1_loss'], color= 'g')
plt.plot(history.history['loss'], color = 'b')

fig.savefig('losses_plot/normalized/losses_' + str(epochs) + '.png', dpi = fig.dpi)
...
for i in range(pred.shape[0]):
	...
	fig.savefig('generated_images/normalized/100/' + str(i) + typ + '_' + str(epochs) + '.png', dpi = fig.dpi, transparent = True)
...
# Give more importance/ability to discriminator to control
